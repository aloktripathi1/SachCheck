...
// Floating claim card shown in hero background
...
export function HeroSection({ onNavigate }: HeroSectionProps) {
  ...
  return (
    ...
    >
      ...
      {/* Main content */}
      <motion.div style={{ y, opacity }} className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 text-center">
        {/* Top badge */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-blue-500/30 bg-blue-500/8 mb-8"
        >
        ...
        >
          <motion.button
            ...
            className="flex items-center gap-2 px-7 py-3.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-base shadow-lg shadow-blue-600/40 transition-colors"
          >
            <Sparkles size={16} />
            Analyze an Article
            <ArrowRight size={15} />
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={() => onNavigate('analyze')}
            className="flex items-center gap-2 px-7 py-3.5 rounded-xl border border-white/[0.12] bg-white/[0.04] text-slate-300 font-semibold text-base hover:bg-white/[0.08] hover:border-white/20 transition-all"
          >
            <Zap size={15} />
            Try Live Demo
          </motion.button>
      ...
      >
        <motion.div
          ...
          className="w-px h-8 bg-gradient-to-b from-slate-600 to-transparent rounded-full"
        />
        <span className="text-[10px] text-slate-700 tracking-widest uppercase">Scroll</span>
      </motion.div>
    </section>
  )
}
